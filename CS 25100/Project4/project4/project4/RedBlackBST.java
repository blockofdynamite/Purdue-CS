...
public class RedBlackBST<Key extends Comparable<Key>, Value> {
    ...
    public ArrayList<Value> top3() {
        ArrayList<Value> top3Songs = new ArrayList<>();
        Node p = maxTopThree(root);
        top3Songs.add(p.val);

        if (p.left == null) {
            p = parent;
            top3Songs.add(parent.val);
        } else {
            p = max(p.left);
            top3Songs.add(p.val);
        }
    ...
    private Node maxTopThree(Node x) {
        if (x.right == null) return x;
        grandParent = parent;
        parent = x;
        return maxTopThree(x.right);
    }
