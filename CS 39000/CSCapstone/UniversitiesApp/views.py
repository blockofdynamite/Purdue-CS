...
def getCourse(request):
    if request.user.is_authenticated():
        ...
        is_member_of_uni = in_university.members.filter(email__exact=request.user.email)
        is_member = in_course.members.filter(email__exact=request.user.email)
        context = {
            'university': in_university,
            'course': in_course,
            'userInCourse': is_member,
            'userInUni': is_member_of_uni,
        }
...
def editStudentsInClass(request):
    if request.user.is_authenticated():
        if request.method == 'POST':
            in_university_name = request.POST.get('name', 'None')
            in_university = models.University.objects.get(name__exact=in_university_name)
            in_course_tag = request.POST.get('course', 'None')
            in_course = in_university.course_set.get(tag__exact=in_course_tag)
            list_students = []
            for member in in_university.members.all():
                if not member.is_student:
                    continue
                is_in_class = False
                for a in in_course.members.all():
                    if member.email == a.email:
                        is_in_class = True
                        break
                checked = request.POST.get(member.email, 'None') is not 'None'
                if checked and not is_in_class:
                    member.course_set.add(in_course)
                elif not checked and is_in_class:
                    member.course_set.remove(in_course)
            ...
            for member in in_university.members.all():
                if not member.is_student:
                    continue
                entity = {"is_in_class": False, "name": member.get_full_name(), "email": member.email}
                for a in in_course.members.all():
                    if member.email == a.email:
                        entity["is_in_class"] = True
                        break
                list_students.append(entity)
            context = {
                'un': in_university_name,
                'cn': in_course_tag,
                'university': in_university,
                'course': in_course,
