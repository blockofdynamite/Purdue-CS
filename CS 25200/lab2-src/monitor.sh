...
function check_arguments {
	...
	if [ "$1" -lt 7 ]; then
		echo "USAGE: "
		echo "$0 {process id} -cpu {utilization percentage} -mem {max mem in kB} {maximum reports} {time interval}"
		exit
	fi
...
function generate_report {
	...
	cd $REPORTS_DIR
	if [[ $(ls | wc -l) -ge $MAXIMUM_REPORTS ]]; then
		ls -t | tail -n 1 | xargs -d '\n' rm -f
	fi
	cd ..
...
function notify
{
	...
	if [[ "$cpu_usage_int" -gt "$CPU_THRESHOLD" ]]; then
		echo "CPU Usage Exceeded - Sending Email" >&2
		cd reports_dir
		message=$(ls -t | tail -n 1 | xargs cat)
		echo $message | mailx -s "CPU Usage Exceeded" $USER
		cd ..
	fi	
	...
	if [[ "$mem_usage_int" -gt "$MEM_THRESHOLD" ]]; then
		echo "Memory Usage Exceeded - Sending Email" >&2
		cd reports_dir
		message=$(ls -t | tail -n 1 | xargs cat)
		echo $message | mailx -s "Memory Usage Exceeded" $USER
		cd ..
	fi	
