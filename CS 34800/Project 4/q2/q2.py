...
query = "SELECT DISTINCT m.title, m.genres FROM movies AS m JOIN ratings AS r ON m.movieid = r.movieid WHERE r.rating = 5 AND m.title LIKE '%(2000)%'"
results = sqlContext.sql(query)

# write data back to HDFS;
# repartition down to 10 partitions to reduce number of HDFS files written;
results.map(lambda p: str(p.title) + "::" + str(p.genres)).repartition(10).saveAsTextFile(sys.argv[2])
