...
query = "SELECT occupation, count(occupation) AS cnt FROM users GROUP BY occupation"
results = sqlContext.sql(query)

# write data back to HDFS;
# repartition down to 10 partitions to reduce number of HDFS files written;
results.map(lambda p: str(p.occupation) + "::" + str(p.cnt)).repartition(10).saveAsTextFile(sys.argv[2])
