...
query = "SELECT DISTINCT title, round(avg(rating), 1) AS avg FROM movies JOIN ratings ON movies.movieid = ratings.movieid WHERE title LIKE '%(1998)%' GROUP BY title"
results = sqlContext.sql(query)

# write data back to HDFS;
# repartition down to 10 partitions to reduce number of HDFS files written;
results.map(lambda p: str(p.title) + "::" + str(p.avg)).repartition(10).saveAsTextFile(sys.argv[2])
